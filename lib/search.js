...
LibSearch.prototype.multiCalls = function (token, optimizedQueryTerms) {
    ...
    return Q.Promise(function (resolve, reject) {
        ...
        _.each(optimizedQueryTerms, function (query) {
            self.call.SearchSubtitles(token, [query])
...
LibSearch.prototype.bestMatch = function (useragent, queryTerms) {
    var self = this;
    self.call = new OS(useragent);

    return Q.Promise(function (resolve, reject) {
        self.optimizeQueryTerms(queryTerms)
            .then(function (optimizedQueryTerms) {
                return self.multiCalls(queryTerms.token, optimizedQueryTerms)
            })
            .then(self.optimizeSubs)
            .then(resolve)
            .catch(reject);
    });
};
